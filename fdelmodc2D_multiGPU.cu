...
int main(int argc, char **argv)
{
	...
	bndPar *d_bnd;//del
	shotPar *d_shot;//del
	float *d_src_nwav;//del
	float *d_rox, *d_roz, *d_l2m, *d_lam, *d_mul;//del
	float *d_tss, *d_tes, *d_tep, *d_p, *d_q, *d_r;//del
	float *d_vx, *d_vz, *d_tzz, *d_txz, *d_txx;//del
	float *d_rec_vx, *d_rec_vz, *d_rec_p;//del
	float *d_rec_txx, *d_rec_tzz, *d_rec_txz;//del
	float *d_rec_pp, *d_rec_ss;//del
	float *d_rec_udp, *d_rec_udvz;//del
	float *d_beam_vx, *d_beam_vz, *d_beam_p;//del
	float *d_beam_txx, *d_beam_tzz, *d_beam_txz;//del
	float *d_beam_pp, *d_beam_ss;//del
	...
	shotPar **h_d_shot;
	float **h_d_src_nwav;
	float **h_d_rox, **h_d_roz, **h_d_l2m, **h_d_lam, **h_d_mul;
	float **h_d_tss, **h_d_tes, **h_d_tep, **h_d_p, **h_d_q, **h_d_r;
	float **h_d_vx, **h_d_vz, **h_d_tzz, **h_d_txz, **h_d_txx;
	float **h_d_rec_vx, **h_d_rec_vz, **h_d_rec_p;
	float **h_d_rec_txx, **h_d_rec_tzz, **h_d_rec_txz;
	float **h_d_rec_pp, **h_d_rec_ss;
	float **h_d_rec_udp, **h_d_rec_udvz;
	float **h_d_beam_vx, **h_d_beam_vz, **h_d_beam_p;
	float **h_d_beam_txx, **h_d_beam_tzz, **h_d_beam_txz;
	float **h_d_beam_pp, **h_d_beam_ss;
...
/* sink sources to value different than zero */
	...
	// Alloc ptr for each GPU
	h_d_mod = (modPar**)malloc(ndev*sizeof(modPar*));
	h_d_rec = (recPar**)malloc(ndev*sizeof(recPar*));
	h_d_sna = (snaPar**)malloc(ndev*sizeof(snaPar*));
	h_d_wav = (wavPar**)malloc(ndev*sizeof(wavPar*));
	h_d_src = (srcPar**)malloc(ndev*sizeof(srcPar*));
	h_d_bnd = (bndPar**)malloc(ndev*sizeof(bndPar*));
	h_d_shot = (shotPar**)malloc(ndev*sizeof(shotPar*));
	h_d_src_nwav = (float**)malloc(ndev*sizeof(float*));
	h_d_rox = (float**)malloc(ndev*sizeof(float*));
	h_d_roz = (float**)malloc(ndev*sizeof(float*)); 
	h_d_l2m = (float**)malloc(ndev*sizeof(float*));
	h_d_lam = (float**)malloc(ndev*sizeof(float*));
	h_d_mul = (float**)malloc(ndev*sizeof(float*));
	h_d_tss = (float**)malloc(ndev*sizeof(float*));
	h_d_tes = (float**)malloc(ndev*sizeof(float*));
	h_d_tep = (float**)malloc(ndev*sizeof(float*));
	h_d_p = (float**)malloc(ndev*sizeof(float*));
	h_d_q = (float**)malloc(ndev*sizeof(float*));
	h_d_r = (float**)malloc(ndev*sizeof(float*));
	h_d_vx  = (float**)malloc(ndev*sizeof(float*));
	h_d_vz = (float**)malloc(ndev*sizeof(float*));
	h_d_tzz = (float**)malloc(ndev*sizeof(float*));
	h_d_txz = (float**)malloc(ndev*sizeof(float*));
	h_d_txx = (float**)malloc(ndev*sizeof(float*));
	h_d_rec_vx = (float**)malloc(ndev*sizeof(float*));
	h_d_rec_vz = (float**)malloc(ndev*sizeof(float*));
	h_d_rec_p = (float**)malloc(ndev*sizeof(float*));
	h_d_rec_txx = (float**)malloc(ndev*sizeof(float*));
	h_d_rec_tzz = (float**)malloc(ndev*sizeof(float*));
	h_d_rec_txz = (float**)malloc(ndev*sizeof(float*));
	h_d_rec_pp = (float**)malloc(ndev*sizeof(float*));
	h_d_rec_ss = (float**)malloc(ndev*sizeof(float*));
	h_d_rec_udp = (float**)malloc(ndev*sizeof(float*));
	h_d_rec_udvz = (float**)malloc(ndev*sizeof(float*));
	h_d_beam_vx = (float**)malloc(ndev*sizeof(float*));
	h_d_beam_vz = (float**)malloc(ndev*sizeof(float*));
	h_d_beam_p = (float**)malloc(ndev*sizeof(float*));
	h_d_beam_txx = (float**)malloc(ndev*sizeof(float*));
	h_d_beam_tzz = (float**)malloc(ndev*sizeof(float*));
	h_d_beam_txz = (float**)malloc(ndev*sizeof(float*));
	h_d_beam_pp = (float**)malloc(ndev*sizeof(float*));
	h_d_beam_ss = (float**)malloc(ndev*sizeof(float*));


	for(idev=0; idev<ndev; idev++){
		cudaSetDevice(idev);
		wrap_cudaMemGetInfo("before alloc");

		// GPU parameter structures: alloc and init	
		cuda_initParStructs(mod, src, wav, bnd, rec, shot, sna,
						 &h_d_mod[idev], &h_d_src[idev], &h_d_wav[idev], &h_d_bnd[idev], &h_d_rec[idev], &h_d_shot[idev], &h_d_sna[idev], verbose);
		...
		// GPU constant coeff fields: alloc and init
		cuda_initCoefFields(mod, rox,    roz,    l2m,    lam,    mul,    tss,    tes,    tep,    p,    q,    r,
							    &h_d_rox[idev], &h_d_roz[idev], &h_d_l2m[idev], &h_d_lam[idev], &h_d_mul[idev], &h_d_tss[idev], &h_d_tes[idev], &h_d_tep[idev], &h_d_p[idev], &h_d_q[idev], &h_d_r[idev], verbose);

		// GPU main fields: alloc and init
		cuda_initMainFields(mod, &h_d_vx[idev], &h_d_vz[idev], &h_d_tzz[idev], &h_d_txz[idev], &h_d_txx[idev], verbose);

		// GPU receivers
		cuda_initRec(mod, rec, &h_d_rec_vx[idev], &h_d_rec_vz[idev], &h_d_rec_p[idev], &h_d_rec_txx[idev], &h_d_rec_tzz[idev], &h_d_rec_txz[idev], 
								&h_d_rec_pp[idev], &h_d_rec_ss[idev], &h_d_rec_udp[idev], &h_d_rec_udvz[idev], verbose);

		// GPU sources array (&h_d_src_nwav)
		cuda_init_d_src_nwav(wav, src_nwav, &h_d_src_nwav[idev], verbose);

		// GPU boundaries
		cuda_init_boundaries(mod, bnd, verbose);

		wrap_cudaMemGetInfo("after alloc");
	}
	

	printf("exiting after cudaMallocs\n");//del
	exit(0);//del
	...
	cuda_wrap_fwrite(d_src_nwav, wav.nst, sizeof(float), "d_src_nwav.bin"); //del
