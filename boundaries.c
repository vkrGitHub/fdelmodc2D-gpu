...
***********************************************************************/
    ...
    // printf("boundariesP free surf ixo ixe izo ize = %d %d %d %d\n",mod.ioPx,mod.iePx,mod.ioPz,mod.iePz);//del
...
#pragma ivdep
                ...
                // printf("boundaries taper vx bnd.tapx[%d] = %f\n",ix-ib,bnd.tapx[ix-ib]);//del
            }
...
#pragma ivdep
                ...
                // printf("boundaries taper vz bnd.tapz[%d] = %f\n",ix-ib,bnd.tapz[ix-ib]);//del
            }
...
#pragma omp for private (ix, iz, dvx, dvz, Jz, ipml) 
          ...
          // printf("boundariesV top PML ixo ixe izo ize = %d %d %d %d\n",mod.ioPx,mod.iePx,mod.ioPz-npml,mod.ioPz);
...
#pragma omp for private (ix, iz, dvx, dvz, Jx, ipml) 
        ...
        // printf("boundariesV P-vx ixo ixe izo ize = %d %d %d %d\n", mod.ioPx-npml, mod.ioPx, mod.ioPz, mod.iePz);
...
#pragma omp for private (ix, iz, dvx, dvz, Jx, ipml) 
            ...
            // printf("boundariesV right PML P ixo ixe izo ize=%d %d %d %d\n",mod.iePx,mod.iePx+npml,mod.ioPz,mod.iePz);
        }
...
#pragma omp for private (ix, iz, dvx, dvz, Jz, ipml)
            ...
            // printf("boundariesV pml p-vx ixo ixe izo ize = %d %d %d %d\n", ixo,ixe,izo,ize);
        }
...
#pragma omp for private (ix) nowait
            ...
            // printf("boundareisV free surf bot ixo ixe=%d %d\n", mod.ioPx, mod.iePx);//del
        }
        if (bnd.lef==1) { /* free surface at left */
#pragma omp for private (iz) nowait
            for (iz=mod.ioPz; iz<mod.iePz; iz++) {
