...
//    if (bnd.top==4 || bnd.top==2) ibndz += bnd.ntap;
	...
	for (is=0; is<rec->n; is++) {
		for (idx=0; idx<=ndot-1; idx++) {
			for (idz=0; idz<=ndot-1; idz++) {
				dum[rec->x[is]*nz+rec->z[is]] = -1.0; //highlight main point // Jan / Joeri check changes
				//dum[(MAX(0,rec->x[is]-1))*nz+rec->z[is]] = -1.0; // highlight x_pos-ndot 
				dum[(MAX(0,rec->x[is]-idx))*nz+rec->z[is]] = -1.0; // highlight x_pos-ndot 
				//dum[(MIN(nx-1,rec->x[is]+1))*nz+rec->z[is]] = -1.0; // highlight x_pos+ndot 
				dum[(MIN(nx-1,rec->x[is]+idx))*nz+rec->z[is]] = -1.0; // highlight x_pos+ndot 
				//dum[rec->x[is]*nz+MAX(0,rec->z[is]-1)] = -1.0; // highlight z_pos-ndot 
				dum[rec->x[is]*nz+MAX(0,rec->z[is]-idz)] = -1.0; // highlight z_pos-ndot 
				//dum[rec->x[is]*nz+MIN(nz-1,rec->z[is]+1)] = -1.0;// highlight z_pos+ndot
				dum[rec->x[is]*nz+MIN(nz-1,rec->z[is]+idz)] = -1.0;// highlight z_pos+ndot
			}
		}
